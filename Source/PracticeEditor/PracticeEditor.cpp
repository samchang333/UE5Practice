...
#include "Private/AssetTypeActions_PracticeContextEffectsLibrary.h"
#include "ToolMenu.h"
#include "ToolMenus.h"
...
DEFINE_LOG_CATEGORY(LogPracticeEditor);
...
{
	return HasNoPlayWorld() && !GetDefault<UPracticeDeveloperSettings>()->CommonEditorMaps.IsEmpty();
}
...
static TSharedRef<SWidget> GetCommonMapsDropdown()
{
	...
	for (const FSoftObjectPath& Path : GetDefault<UPracticeDeveloperSettings>()->CommonEditorMaps)
	{
		if (!Path.IsValid())
		{
			continue;
...
/**
 * FPracticeEditorModule
 */
class FPracticeEditorModule : public FDefaultGameModuleImpl
{
	typedef FPracticeEditorModule ThisClass;
	...
	virtual void StartupModule() override
	{
		...
		if (!IsRunningGame())
		{
			FModuleManager::Get().OnModulesChanged().AddRaw(this, &FPracticeEditorModule::ModulesChangedCallback);
		...
		AssetTools.RegisterAssetTypeActions(MakeShareable(new FAssetTypeActions_PracticeContextEffectsLibrary));
	}

	void OnBeginPIE(bool bIsSimulating)
	{
		UPracticeExperienceManager* ExperienceManager = GEngine->GetEngineSubsystem<UPracticeExperienceManager>();
...
IMPLEMENT_MODULE(FPracticeEditorModule, PracticeEditor);
