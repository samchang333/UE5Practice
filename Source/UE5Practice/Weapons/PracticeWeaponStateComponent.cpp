...
#include "PracticeWeaponStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeWeaponStateComponent)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gameplay_Zone, "Gameplay.Zone");

UPracticeWeaponStateComponent::UPracticeWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void UPracticeWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (APawn* Pawn = GetPawn<APawn>())
	{
		...
		{
			if (UPracticeRangedWeaponInstance* CurrentWeapon = Cast<UPracticeRangedWeaponInstance>(EquipmentManager->GetFirstInstanceOfType(UPracticeRangedWeaponInstance::StaticClass())))
			{
...
bool UPracticeWeaponStateComponent::ShouldShowHitAsSuccess(const FHitResult& Hit) const
{
	AActor* HitActor = Hit.GetActor();

	//@TODO: Don't treat a hit that dealt no damage (due to invulnerability or similar) as a success
	UWorld* World = GetWorld();
	if (UPracticeTeamSubsystem* TeamSubsystem = UWorld::GetSubsystem<UPracticeTeamSubsystem>(GetWorld()))
	{
...
bool UPracticeWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void UPracticeWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
	for (int i = 0; i < UnconfirmedServerSideHitMarkers.Num(); i++)
	{
		FPracticeServerSideHitMarkerBatch& Batch = UnconfirmedServerSideHitMarkers[i];
		if (Batch.UniqueId == UniqueId)
		{
			if (bSuccess && (HitReplaces.Num() != Batch.Markers.Num()))
			{
				...
				for (const FPracticeScreenSpaceHitLocation& Entry : Batch.Markers)
				{
...
void UPracticeWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	FPracticeServerSideHitMarkerBatch& NewUnconfirmedHitMarker = UnconfirmedServerSideHitMarkers.Emplace_GetRef(InTargetData.UniqueId);
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FPracticeScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
...
void UPracticeWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void UPracticeWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double UPracticeWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();
