...
#include "PracticeWeaponStateComponent.generated.h"
...
struct FPracticeScreenSpaceHitLocation
{
...
struct FPracticeServerSideHitMarkerBatch
{
	FPracticeServerSideHitMarkerBatch() { }

	FPracticeServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FPracticeScreenSpaceHitLocation> Markers;
...
UCLASS()
class UPracticeWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UPracticeWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FPracticeScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FPracticeScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FPracticeServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
