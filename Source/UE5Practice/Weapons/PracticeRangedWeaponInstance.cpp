...
#include "PracticeRangedWeaponInstance.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Practice_Weapon_SteadyAimingCamera, "Practice.Weapon.SteadyAimingCamera");

UPracticeRangedWeaponInstance::UPracticeRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UPracticeRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UPracticeRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UPracticeRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UPracticeRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UPracticeRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UPracticeRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UPracticeRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UPracticeRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UPracticeRangedWeaponInstance::AddSpread()
{
...
float UPracticeRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UPracticeRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UPracticeRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UPracticeRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UPracticeCameraComponent* CameraComponent = UPracticeCameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_Practice_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
