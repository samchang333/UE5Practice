...
#include "PracticeUserFacingExperienceDefinition.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeUserFacingExperienceDefinition)

namespace Practice::Experience
{
	UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Platform_Trait_ReplaySupport, "Platform.Trait.ReplaySupport");
};

UCommonSession_HostSessionRequest* UPracticeUserFacingExperienceDefinition::CreateHostingRequest() const
{
	...
	if (ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(Practice::Experience::TAG_Platform_Trait_ReplaySupport.GetTag()))
	{
		if (bRecordReplay)
		{
