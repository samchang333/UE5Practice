...
#include "PracticeNumberPopComponent.generated.h"

class UObject;
struct FFrame;

USTRUCT(BlueprintType)
struct FPracticeNumberPopRequest
{
	GENERATED_BODY()

	// The world location to create the number pop at
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Practice|Number Pops")
...
UCLASS(Abstract)
class UPracticeNumberPopComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UPracticeNumberPopComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	/** Adds a damage number to the damage number list for visualization */
	UFUNCTION(BlueprintCallable, Category = Foo)
	virtual void AddNumberPop(const FPracticeNumberPopRequest& NewRequest) {}
};
