...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeContextEffectsLibrary)


void UPracticeContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UPracticeContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UPracticeContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UPracticeContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FPracticeContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UPracticeActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FPracticeContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UPracticeActiveContextEffects* NewActiveContextEffects = NewObject<UPracticeActiveContextEffects>(this);
	...
	this->PracticeContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UPracticeContextEffectsLibrary::PracticeContextEffectLibraryLoadingComplete(
	TArray<UPracticeActiveContextEffects*> PracticeActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(PracticeActiveContextEffects);
}
