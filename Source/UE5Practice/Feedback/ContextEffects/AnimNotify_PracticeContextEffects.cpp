...
#include "AnimNotify_PracticeContextEffects.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_PracticeContextEffects)



UAnimNotify_PracticeContextEffects::UAnimNotify_PracticeContextEffects()
{
}

void UAnimNotify_PracticeContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_PracticeContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_PracticeContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_PracticeContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> PracticeContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UPracticeContextEffectsInterface>())
			{
				// If so, add it to the Array
				PracticeContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UPracticeContextEffectsInterface>())
					{
						PracticeContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* PracticeContextEffectImplementingObject : PracticeContextEffectImplementingObjects)
			{
				if (PracticeContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IPracticeContextEffectsInterface::Execute_AnimMotionEffect(PracticeContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UPracticeContextEffectLibrary type
						if (UPracticeContextEffectsLibrary* EffectLibrary = Cast<UPracticeContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_PracticeContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_PracticeContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
