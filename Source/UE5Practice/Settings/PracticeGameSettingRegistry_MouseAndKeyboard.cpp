...
#include "CustomSettings/PracticeSettingKeyboardInput.h"
...
UGameSettingCollection* UPracticeGameSettingRegistry::InitializeMouseAndKeyboardSettings(UPracticeLocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetMouseSensitivityX());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetMouseSensitivityY());
			Setting->SetDisplayFormat(UGameSettingValueScalarDynamic::RawTwoDecimals);
			Setting->SetSourceRangeAndStep(TRange<double>(0, 10), 0.01);
			Setting->SetMinimumLimit(0.01);
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetTargetingMultiplier());
			Setting->SetDisplayFormat(UGameSettingValueScalarDynamic::RawTwoDecimals);
			Setting->SetSourceRangeAndStep(TRange<double>(0, 10), 0.01);
			Setting->SetMinimumLimit(0.01);
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetInvertVerticalAxis());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetInvertHorizontalAxis());
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			for (const FLoadedMappableConfigPair& InputConfigPair : RegisteredConfigs)
			{
				...
				{
					UE_LOG(LogPracticeGameSettingRegistry, Warning, TEXT("PlayerMappableInputConfig '%s' has no player mappable keys in it! Skipping it in the setting registry..."), *InputConfigPair.Config->GetConfigName().ToString());
					continue;
				}
				...
				{
					UPracticeSettingKeyboardInput* ExistingSetting = nullptr;
					...
					{
						UE_LOG(LogPracticeGameSettingRegistry, Warning, TEXT("A setting with the name '%s' from config '%s' has already been added! Please remove duplicate name."), *Mapping.PlayerMappableOptions.Name.ToString(), *InputConfigPair.Config->GetConfigName().ToString());
						continue;
					}
					
					for (UGameSetting* Setting : ConfigSettingCollection->GetChildSettings())
					{
						UPracticeSettingKeyboardInput* PracticeKeyboardSetting = Cast<UPracticeSettingKeyboardInput>(Setting);
						if (PracticeKeyboardSetting->GetSettingDisplayName().EqualToCaseIgnored(Mapping.PlayerMappableOptions.DisplayName))
						{
							ExistingSetting = PracticeKeyboardSetting;
							break;
						}
					}
					...
					if (MappingSynthesized.PlayerMappableOptions.Name != NAME_None && !MappingSynthesized.PlayerMappableOptions.DisplayName.IsEmpty())
					{
						...
						UPracticeSettingKeyboardInput* InputBinding = ExistingSetting ? ExistingSetting : NewObject<UPracticeSettingKeyboardInput>();
					...
					{
						UE_LOG(LogPracticeGameSettingRegistry, Warning, TEXT("A setting with the name '%s' from config '%s' could not be added, one of its names is empty!"), *Mapping.PlayerMappableOptions.Name.ToString(), *InputConfigPair.Config->GetConfigName().ToString());
						ensure(false);
					}
				}
			}
