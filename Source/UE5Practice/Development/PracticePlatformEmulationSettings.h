...
#include "PracticePlatformEmulationSettings.generated.h"
...
class UPracticePlatformEmulationSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UPracticePlatformEmulationSettings();
...
private:
	...
	// The base device profile to pretend we are using when emulating device-specific device profiles applied from UPracticeSettingsLocal
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="Practice.Settings.ApplyFrameRateSettingsInPIE"))
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="Practice.Settings.ApplyFrontEndPerformanceOptionsInPIE"))
	bool bApplyFrontEndPerformanceOptionsInPIE = false;

	// Should we apply experience/platform emulated device profiles in PIE?
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(InlineEditConditionToggle, ConsoleVariable="Practice.Settings.ApplyDeviceProfilesInPIE"))
