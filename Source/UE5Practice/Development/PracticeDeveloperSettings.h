...
#include "PracticeDeveloperSettings.generated.h"
...
class UPracticeDeveloperSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UPracticeDeveloperSettings();
...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=Practice, meta=(AllowedTypes="PracticeExperienceDefinition"))
	FPrimaryAssetId ExperienceOverride;

	UPROPERTY(BlueprintReadOnly, config, Category=PracticeBots)
	bool bOverrideBotCount = false;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PracticeBots, meta=(EditCondition=bOverrideBotCount))
	int32 OverrideNumPlayerBotsToSpawn = 0;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PracticeBots)
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=Practice)
	bool bTestFullGameFlowInPIE = false;

	/**
	* Should force feedback effects be played, even if the last input device was not a gamepad?
	* The default behavior in Practice is to only play force feedback if the most recent input device was a gamepad.
	*/
	UPROPERTY(config, EditAnywhere, Category = Practice, meta = (ConsoleVariable = "PracticePC.ShouldAlwaysPlayForceFeedback"))
	...
	UPROPERTY(config, EditAnywhere, Category=Practice)
	TArray<FPracticeCheatToRun> CheatsToRun;
...
public:
	...
	UE5PRACTICE_API void OnPlayInEditorStarted() const;
