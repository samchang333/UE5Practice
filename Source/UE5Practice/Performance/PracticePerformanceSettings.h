...
#include "PracticePerformanceSettings.generated.h"
...
USTRUCT()
struct FPracticeQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FPracticePerformanceStatGroup
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere)
	TSet<EPracticeDisplayablePerformanceStat> AllowedStats;
};

// How hare frame pacing and overall graphics settings controlled/exposed for the platform?
UENUM()
enum class EPracticeFramePacingMode : uint8
{
...
class UPracticePlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UPracticePlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UPracticePlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FPracticeQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	EPracticeFramePacingMode FramePacingMode = EPracticeFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by Practice.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==EPracticeFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="Practice Performance Settings"))
class UPracticePerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UPracticePerformanceSettings();
...
public:
	...
	TArray<FPracticePerformanceStatGroup> UserFacingPerformanceStats;
};
