...
#include "PracticeFrontendStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeFrontendStateComponent)
...
UPracticeFrontendStateComponent::UPracticeFrontendStateComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UPracticeFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UPracticeFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool UPracticeFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void UPracticeFrontendStateComponent::OnExperienceLoaded(const UPracticeExperienceDefinition* Experience)
{
...
void UPracticeFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void UPracticeFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &UPracticeFrontendStateComponent::OnUserInitialized);
...
void UPracticeFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &UPracticeFrontendStateComponent::OnUserInitialized);
...
void UPracticeFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void UPracticeFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
