...
#include "PracticeCameraMode_ThirdPerson.h"
#include "Camera/PracticeCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/PracticePenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "PracticeCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeCameraMode_ThirdPerson)

namespace PracticeCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UPracticeCameraMode_ThirdPerson::UPracticeCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FPracticePenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FPracticePenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FPracticePenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FPracticePenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FPracticePenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FPracticePenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FPracticePenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UPracticeCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UPracticeCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UPracticeCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UPracticeCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IPracticeCameraAssistInterface* TargetControllerAssist = Cast<IPracticeCameraAssistInterface>(TargetController);

	IPracticeCameraAssistInterface* TargetActorAssist = Cast<IPracticeCameraAssistInterface>(TargetActor);
	...
	IPracticeCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IPracticeCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IPracticeCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IPracticeCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UPracticeCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IPracticeCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FPracticePenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(PracticeCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UPracticeCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UPracticeCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
