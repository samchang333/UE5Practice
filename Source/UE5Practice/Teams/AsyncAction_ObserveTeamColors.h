...
class IPracticeTeamAgentInterface;
class UPracticeTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UPracticeTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements IPracticeTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UPracticeTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UPracticeTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<IPracticeTeamAgentInterface> TeamInterfacePtr;
