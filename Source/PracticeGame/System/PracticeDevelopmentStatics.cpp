...
#include "PracticeDevelopmentStatics.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeDevelopmentStatics)

bool UPracticeDevelopmentStatics::ShouldSkipDirectlyToGameplay()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UPracticeDeveloperSettings>()->bTestFullGameFlowInPIE;
	}
#endif
	return false;
}

bool UPracticeDevelopmentStatics::ShouldLoadCosmeticBackgrounds()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UPracticeDeveloperSettings>()->bSkipLoadingCosmeticBackgroundsInPIE;
	}
#endif
	return true;
}

bool UPracticeDevelopmentStatics::CanPlayerBotsAttack()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return GetDefault<UPracticeDeveloperSettings>()->bAllowPlayerBotsToAttack;
	}
#endif
	return true;
}
...
UWorld* UPracticeDevelopmentStatics::FindPlayInEditorAuthorityWorld()
{
	check(GEngine);
...
TArray<FAssetData> UPracticeDevelopmentStatics::GetAllBlueprints()
{
...
UClass* UPracticeDevelopmentStatics::FindBlueprintClass(const FString& TargetNameRaw, UClass* DesiredBaseClass)
{
	FString TargetName = TargetNameRaw;
	TargetName.RemoveFromEnd(TEXT("_C"), ESearchCase::CaseSensitive);

	TArray<FAssetData> BlueprintList = UPracticeDevelopmentStatics::GetAllBlueprints();
...
UClass* UPracticeDevelopmentStatics::FindClassByShortName(const FString& SearchToken, UClass* DesiredBaseClass, bool bLogFailures)
{
