...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeGameInstance)

namespace Practice
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarPracticeTestEncryption(
		TEXT("Practice.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarPracticeUseDTLSEncryption(
		TEXT("Practice.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using Practice.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarPracticeTestDTLSFingerprint(
		TEXT("Practice.TestDTLSFingerprint"),
...
UPracticeGameInstance::UPracticeGameInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UPracticeGameInstance::Init()
{
	Super::Init();
	...
	{
		ComponentManager->RegisterInitState(PracticeGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(PracticeGameplayTags::InitState_DataAvailable, false, PracticeGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(PracticeGameplayTags::InitState_DataInitialized, false, PracticeGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(PracticeGameplayTags::InitState_GameplayReady, false, PracticeGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UPracticeGameInstance::OnPreClientTravelToSession);
	}
}

void UPracticeGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
APracticePlayerController* UPracticeGameInstance::GetPrimaryPlayerController() const
{
	return Cast<APracticePlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UPracticeGameInstance::CanJoinRequestedSession() const
{
...
void UPracticeGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (Practice::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (Practice::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("PracticeTest.pem");
...
void UPracticeGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UPracticeGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (Practice::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (Practice::bUseDTLSEncryption)
		{
