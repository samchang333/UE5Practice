...
#include "PracticeTeamSubsystem.generated.h"

class AActor;
class APracticePlayerState;
class APracticeTeamInfoBase;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnPracticeTeamDisplayAssetChangedDelegate, const UPracticeTeamDisplayAsset*, DisplayAsset);

USTRUCT()
struct FPracticeTeamTrackingInfo
{
	GENERATED_BODY()

public:
	UPROPERTY()
	TObjectPtr<APracticeTeamPublicInfo> PublicInfo = nullptr;

	UPROPERTY()
	TObjectPtr<APracticeTeamPrivateInfo> PrivateInfo = nullptr;

	UPROPERTY()
	TObjectPtr<UPracticeTeamDisplayAsset> DisplayAsset = nullptr;

	UPROPERTY()
	FOnPracticeTeamDisplayAssetChangedDelegate OnTeamDisplayAssetChanged;

public:
	void SetTeamInfo(APracticeTeamInfoBase* Info);
	void RemoveTeamInfo(APracticeTeamInfoBase* Info);
};

// Result of comparing the team affiliation for two actors
UENUM(BlueprintType)
enum class EPracticeTeamComparison : uint8
{
...
UCLASS()
class PRACTICEGAME_API UPracticeTeamSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
public:
	...
	bool RegisterTeamInfo(APracticeTeamInfoBase* TeamInfo);

	// Tries to unregister a team, will return false if it didn't work
	bool UnregisterTeamInfo(APracticeTeamInfoBase* TeamInfo);
	...
	const APracticePlayerState* FindPlayerStateFromActor(const AActor* PossibleTeamActor) const;
	...
	EPracticeTeamComparison CompareTeams(const UObject* A, const UObject* B, int32& TeamIdA, int32& TeamIdB) const;

	// Compare the teams of two actors and returns a value indicating if they are on same teams, different teams, or one/both are invalid
	EPracticeTeamComparison CompareTeams(const UObject* A, const UObject* B) const;
	...
	UFUNCTION(BlueprintCallable, Category=Teams)
	UPracticeTeamDisplayAsset* GetTeamDisplayAsset(int32 TeamId, int32 ViewerTeamId);

	// Gets the team display asset for the specified team, from the perspective of the specified team
	// (You have to specify a viewer too, in case the game mode is in a 'local player is always blue team' sort of situation)
	UFUNCTION(BlueprintCallable, Category = Teams)
	UPracticeTeamDisplayAsset* GetEffectiveTeamDisplayAsset(int32 TeamId, UObject* ViewerTeamAgent);
	...
	void NotifyTeamDisplayAssetModified(UPracticeTeamDisplayAsset* ModifiedAsset);

	// Register for a team display asset notification for the specified team ID
	FOnPracticeTeamDisplayAssetChangedDelegate& GetTeamDisplayAssetChangedDelegate(int32 TeamId);

private:
	UPROPERTY()
	TMap<int32, FPracticeTeamTrackingInfo> TeamMap;
