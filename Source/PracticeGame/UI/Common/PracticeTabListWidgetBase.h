...
#include "PracticeTabListWidgetBase.generated.h"
...
struct FPracticeTabDescriptor
{
	GENERATED_BODY()

public:
	FPracticeTabDescriptor()
	: bHidden(false)
...
class UPracticeTabButtonInterface : public UInterface
{
	GENERATED_BODY()
};

class IPracticeTabButtonInterface
{
	GENERATED_BODY()

public:
	UFUNCTION(BlueprintNativeEvent, Category = "Tab Button")
	void SetTabLabelInfo(const FPracticeTabDescriptor& TabDescriptor);
};

UCLASS(Blueprintable, BlueprintType, Abstract, meta = (DisableNativeTick))
class PRACTICEGAME_API UPracticeTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
	
public:
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Tab List")
	bool GetPreregisteredTabInfo(const FName TabNameId, FPracticeTabDescriptor& OutTabInfo);

	/** Helper method to get at all the preregistered tab infos */
	const TArray<FPracticeTabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
	...
	UFUNCTION(BlueprintCallable, Category = "Tab List")
	bool RegisterDynamicTab(const FPracticeTabDescriptor& TabDescriptor);
	...
	DECLARE_EVENT_TwoParams(UPracticeTabListWidgetBase, FOnTabContentCreatedNative, FName /* TabId */, UCommonUserWidget* /* TabWidget */);
...
private:
	...
	TArray<FPracticeTabDescriptor> PreregisteredTabInfoArray;
	 ...
	 */
	UPROPERTY()
	TMap<FName, FPracticeTabDescriptor> PendingTabLabelInfoMap;
};
