...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeTabListWidgetBase)

void UPracticeTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UPracticeTabListWidgetBase::NativeConstruct()
{
...
void UPracticeTabListWidgetBase::NativeDestruct()
{
	for (FPracticeTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UPracticeTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FPracticeTabDescriptor& OutTabInfo)
{
	const FPracticeTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FPracticeTabDescriptor& TabInfo) -> bool
	{
...
void UPracticeTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FPracticeTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UPracticeTabListWidgetBase::RegisterDynamicTab(const FPracticeTabDescriptor& TabDescriptor)
{
...
void UPracticeTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FPracticeTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UPracticeTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void UPracticeTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FPracticeTabDescriptor* TabInfoPtr = nullptr;
	
	FPracticeTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UPracticeTabButtonInterface::StaticClass()))
	{
		...
		{
			IPracticeTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UPracticeTabListWidgetBase::IsFirstTabActive() const
{
...
bool UPracticeTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UPracticeTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 UPracticeTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UPracticeTabListWidgetBase::SetupTabs()
{
	for (FPracticeTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
