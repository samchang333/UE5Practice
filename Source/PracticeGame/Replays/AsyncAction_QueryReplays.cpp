...
void UAsyncAction_QueryReplays::Activate()
{
	...
	ResultList = NewObject<UPracticeReplayList>();
...
void UAsyncAction_QueryReplays::OnEnumerateStreamsComplete(const FEnumerateStreamsResult& Result)
{
	...
	{
		UPracticeReplayListEntry* NewReplayEntry = NewObject<UPracticeReplayListEntry>(ResultList);
	...
	{
		FORCEINLINE bool operator()(const UPracticeReplayListEntry& A, const UPracticeReplayListEntry& B) const
		{
