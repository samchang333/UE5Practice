...
#include "PracticeCheatManager.generated.h"
...
DECLARE_LOG_CATEGORY_EXTERN(LogPracticeCheat, Log, All);


/**
 * UPracticeCheatManager
 *
 *	Base cheat manager class used by this project.
 */
UCLASS(config = Game, Within = PlayerController, MinimalAPI)
class UPracticeCheatManager : public UCheatManager
{
	GENERATED_BODY()
...
protected:
	...
	void ApplySetByCallerDamage(UPracticeAbilitySystemComponent* PracticeASC, float DamageAmount);
	void ApplySetByCallerHeal(UPracticeAbilitySystemComponent* PracticeASC, float HealAmount);

	UPracticeAbilitySystemComponent* GetPlayerAbilitySystemComponent() const;
};
