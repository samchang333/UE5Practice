...
#include "PracticeSafeZoneEditor.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeSafeZoneEditor)
...
UPracticeSafeZoneEditor::UPracticeSafeZoneEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UPracticeSafeZoneEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void UPracticeSafeZoneEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(UPracticeSettingsLocal::Get()->GetSafeZone());
	
	Button_Done->OnClicked().AddUObject(this, &UPracticeSafeZoneEditor::HandleDoneClicked);

	Button_Back->SetVisibility((bCanCancel)? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	if (bCanCancel)
	{
		Button_Back->OnClicked().AddUObject(this, &UPracticeSafeZoneEditor::HandleBackClicked);
	}
}

bool UPracticeSafeZoneEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply UPracticeSafeZoneEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply UPracticeSafeZoneEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UPracticeSafeZoneEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void UPracticeSafeZoneEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(UPracticeSettingsLocal::Get()->GetSafeZone());
}

void UPracticeSafeZoneEditor::HandleDoneClicked()
{
