...
#include "PracticeSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EPracticeAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UPracticeSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 *               maybe change Localsettings to PracticePlatformSettings, or DeviceSettings?  Make this one UserSettings?  TBD
 *               
...
class UPracticeSettingsShared : public USaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UPracticeSettingsShared, FOnSettingChangedEvent, UPracticeSettingsShared* Settings);
	FOnSettingChangedEvent OnSettingChanged;

public:

	UPracticeSettingsShared();

	void Initialize(UPracticeLocalPlayer* LocalPlayer);

	bool IsDirty() const { return bIsDirty; }
	void ClearDirtyFlag() { bIsDirty = false; }

	void SaveSettings();
	static UPracticeSettingsShared* LoadOrCreateSettings(const UPracticeLocalPlayer* LocalPlayer);
...
public:
	UFUNCTION()
	EPracticeAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EPracticeAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EPracticeAllowBackgroundAudioSetting AllowAudioInBackground = EPracticeAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EPracticeGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EPracticeGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EPracticeGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EPracticeGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EPracticeGamepadSensitivity GamepadLookSensitivityPreset = EPracticeGamepadSensitivity::Normal;
	UPROPERTY()
	EPracticeGamepadSensitivity GamepadTargetingSensitivityPreset = EPracticeGamepadSensitivity::Normal;
...
private:
	...
	UPROPERTY(Transient)
	TObjectPtr<UPracticeLocalPlayer> OwningPlayer = nullptr;
};
