...
UGameSettingCollection* UPracticeGameSettingRegistry::InitializeGamepadSettings(UPracticeLocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			if (UCommonInputPlatformSettings* PlatformInputSettings = UPlatformSettingsManager::Get().GetSettingsForPlatform<UCommonInputPlatformSettings>())
			{
				...
				if (Setting->GetDynamicOptions().Num() > 1 && PlatformInputSettings->CanChangeGamepadType())
				{
					...
					const FName CurrentControllerPlatform = GetDefault<UPracticeSettingsLocal>()->GetControllerPlatform();
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetForceFeedbackEnabled());
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetGamepadLookSensitivityPreset());

			for (int32 PresetIndex = 1; PresetIndex < (int32)EPracticeGamepadSensitivity::MAX; PresetIndex++)
			{
				Setting->AddEnumOption(static_cast<EPracticeGamepadSensitivity>(PresetIndex), EGamepadSensitivityText[PresetIndex]);
			}
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetGamepadTargetingSensitivityPreset());
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetGamepadMoveStickDeadZone());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UPracticeSettingsShared>()->GetGamepadLookStickDeadZone());
			Setting->SetDisplayFormat(UGameSettingValueScalarDynamic::ZeroToOnePercent);
			Setting->SetMinimumLimit(0.05);
			Setting->SetMaximumLimit(0.95);
