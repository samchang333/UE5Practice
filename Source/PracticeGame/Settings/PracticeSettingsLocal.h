...
#include "PracticeSettingsLocal.generated.h"
...
USTRUCT()
struct FPracticeScalabilitySnapshot
{
	GENERATED_BODY()

	FPracticeScalabilitySnapshot();
...
/**
 * UPracticeSettingsLocal
 */
UCLASS()
class UPracticeSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	static UPracticeSettingsLocal* Get();
...
public:
	...
	EPracticeStatDisplayMode GetPerfStatDisplayState(EPracticeDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EPracticeDisplayablePerformanceStat Stat, EPracticeStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UPracticeSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EPracticeDisplayablePerformanceStat, EPracticeStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FPracticeScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FPracticeScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UPracticeSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
public:
	...
	DECLARE_EVENT_OneParam(UPracticeSettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	 ...
	 */
	void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UPracticeLocalPlayer* LocalPlayer);
	 ...
	 * @param LocalPlayer   local player to reset the keybinding on
	 */
	void ResetKeybindingToDefault(const FName MappingName, UPracticeLocalPlayer* LocalPlayer);

	/** Resets all keybindings to their default value in their input mapping context
	 * @param LocalPlayer   local player to reset the keybinding on
	 */
	void ResetKeybindingsToDefault(UPracticeLocalPlayer* LocalPlayer);
