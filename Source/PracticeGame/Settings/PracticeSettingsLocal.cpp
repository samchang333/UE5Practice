...
#include "Audio/PracticeAudioSettings.h"
#include "Audio/PracticeAudioMixEffectsSubsystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Practice.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Practice.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Practice.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Practice.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Practice.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Practice.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Practice.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Practice.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Practice.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Practice.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FPracticeScalabilitySnapshot::FPracticeScalabilitySnapshot()
{
...
namespace PracticeSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FPracticeScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
		...
		Mode = FPracticeScalabilitySnapshot();
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UPracticeSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UPracticeSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UPracticeSettingsLocal::UPracticeSettingsLocal()
{
...
void UPracticeSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UPracticeSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UPracticePlatformSpecificRenderingSettings* PlatformSettings = UPracticePlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EPracticeFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	PracticeSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UPracticeSettingsLocal::ResetToCurrentSettings()
{
...
void UPracticeSettingsLocal::BeginDestroy()
{
...
UPracticeSettingsLocal* UPracticeSettingsLocal::Get()
{
	return GEngine ? CastChecked<UPracticeSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UPracticeSettingsLocal::ConfirmVideoMode()
{
...
float UPracticeSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UPracticePlatformSpecificRenderingSettings* PlatformSettings = UPracticePlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EPracticeFramePacingMode::DesktopStyle)
	{
...
int32 UPracticeSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return PracticeSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UPracticeSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FPracticeScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UPracticeSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UPracticeSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UPracticeSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UPracticeSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UPracticeSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UPracticeSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EPracticeStatDisplayMode UPracticeSettingsLocal::GetPerfStatDisplayState(EPracticeDisplayablePerformanceStat Stat) const
{
	if (const EPracticeStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EPracticeStatDisplayMode::Hidden;
	}
}

void UPracticeSettingsLocal::SetPerfStatDisplayState(EPracticeDisplayablePerformanceStat Stat, EPracticeStatDisplayMode DisplayMode)
{
	if (DisplayMode == EPracticeStatDisplayMode::Hidden)
	{
...
float UPracticeSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UPracticeSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UPracticeSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UPracticeSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UPracticeSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UPracticeSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UPracticeSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UPracticeSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UPracticeSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UPracticeSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UPracticeSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UPracticeSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UPracticeSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UPracticeSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UPracticeSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UPracticeSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return PracticeSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UPracticeSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return PracticeSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UPracticeSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UPracticeSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UPracticePlatformSpecificRenderingSettings* PlatformSettings = UPracticePlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EPracticeFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return PracticeSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UPracticeSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UPracticePlatformSpecificRenderingSettings* PlatformSettings = UPracticePlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EPracticeFramePacingMode::MobileStyle)
	{
...
void UPracticeSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UPracticeSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UPracticePlatformSpecificRenderingSettings* PlatformSettings = UPracticePlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EPracticeFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = PracticeSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UPracticeSettingsLocal::ClampMobileQuality()
{
	const UPracticePlatformSpecificRenderingSettings* PlatformSettings = UPracticePlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EPracticeFramePacingMode::MobileStyle)
	{
...
void UPracticeSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = PracticeSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UPracticeSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = PracticeSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = PracticeSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UPracticeSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UPracticeSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UPracticeSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UPracticeSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UPracticeSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = PracticeSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UPracticeSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UPracticeSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UPracticeAudioMixEffectsSubsystem* PracticeAudioMixEffectsSubsystem = World->GetSubsystem<UPracticeAudioMixEffectsSubsystem>())
			{
				PracticeAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UPracticeSettingsLocal::CanRunAutoBenchmark() const
{
	const UPracticePlatformSpecificRenderingSettings* PlatformSettings = UPracticePlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UPracticeSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UPracticeSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UPracticeSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UPracticeSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UPracticeSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UPracticeSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UPracticeSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UPracticeSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UPracticeSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UPracticeSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UPracticeSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UPracticeSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UPracticeSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UPracticeSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UPracticeSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UPracticeSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UPracticeSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UPracticeSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UPracticeSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EPracticeFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = PracticeSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UPracticeSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UPracticeSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UPracticeSettingsLocal::RegisterInputConfig(ECommonInputType Type, const UPlayerMappableInputConfig* NewConfig, const bool bIsActive)
{
	if (NewConfig)
	{
...
int32 UPracticeSettingsLocal::UnregisterInputConfig(const UPlayerMappableInputConfig* ConfigToRemove)
{
	if (ConfigToRemove)
	{
...
const UPlayerMappableInputConfig* UPracticeSettingsLocal::GetInputConfigByName(FName ConfigName) const
{
...
void UPracticeSettingsLocal::GetRegisteredInputConfigsOfType(ECommonInputType Type, TArray<FLoadedMappableConfigPair>& OutArray) const
{
	OutArray.Empty();
...
void UPracticeSettingsLocal::GetAllMappingNamesFromKey(const FKey InKey, TArray<FName>& OutActionNames)
{
...
void UPracticeSettingsLocal::AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UPracticeLocalPlayer* LocalPlayer)
{
...
void UPracticeSettingsLocal::ResetKeybindingToDefault(const FName MappingName, UPracticeLocalPlayer* LocalPlayer)
{
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
	{
...
void UPracticeSettingsLocal::ResetKeybindingsToDefault(UPracticeLocalPlayer* LocalPlayer)
{
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
	{
...
void UPracticeSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UPracticeAudioSettings* PracticeAudioSettings = GetDefault<UPracticeAudioSettings>())
			{
				...
				if (UObject* ObjPath = PracticeAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from Practice Audio Settings."));
					}
				}

				if (UObject* ObjPath = PracticeAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from Practice Audio Settings."));
					}
				}

				if (UObject* ObjPath = PracticeAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from Practice Audio Settings."));
					}
				}

				if (UObject* ObjPath = PracticeAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from Practice Audio Settings."));
					}
				}

				if (UObject* ObjPath = PracticeAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from Practice Audio Settings."));
					}
				}

				if (UObject* ObjPath = PracticeAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Practice Audio Settings."));
					}
				}
			}
		}
...
void UPracticeSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UPracticeSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UPracticePlatformSpecificRenderingSettings* PlatformSettings = UPracticePlatformSpecificRenderingSettings::Get();
	const TArray<FPracticeQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FPracticeQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UPracticePlatformEmulationSettings* Settings = GetDefault<UPracticePlatformEmulationSettings>();
	...
	{
	case EPracticeFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EPracticeFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EPracticeFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UPracticeSettingsLocal::UpdateConsoleFramePacing()
{
...
void UPracticeSettingsLocal::UpdateDesktopFramePacing()
{
...
void UPracticeSettingsLocal::UpdateMobileFramePacing()
{
...
void UPracticeSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
