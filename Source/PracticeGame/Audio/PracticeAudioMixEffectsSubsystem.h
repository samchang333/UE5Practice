...
#include "PracticeAudioMixEffectsSubsystem.generated.h"
...
USTRUCT()
struct FPracticeAudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 */
UCLASS()
class PRACTICEGAME_API UPracticeAudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	// Default Sound Control Bus Mix retrieved from the Practice Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> DefaultBaseMix = nullptr;

	// Loading Screen Sound Control Bus Mix retrieved from the Practice Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> LoadingScreenMix = nullptr;

	// User Sound Control Bus Mix retrieved from the Practice Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> UserMix = nullptr;

	// Overall Sound Control Bus retrieved from the Practice Audio Settings and linked to the UI and game settings in PracticeSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> OverallControlBus = nullptr;

	// Music Sound Control Bus retrieved from the Practice Audio Settings and linked to the UI and game settings in PracticeSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> MusicControlBus = nullptr;

	// SoundFX Sound Control Bus retrieved from the Practice Audio Settings and linked to the UI and game settings in PracticeSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> SoundFXControlBus = nullptr;

	// Dialogue Sound Control Bus retrieved from the Practice Audio Settings and linked to the UI and game settings in PracticeSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> DialogueControlBus = nullptr;

	// VoiceChat Sound Control Bus retrieved from the Practice Audio Settings and linked to the UI and game settings in PracticeSettingsLocal
	UPROPERTY(Transient)
	...
	UPROPERTY(Transient)
	TArray<FPracticeAudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FPracticeAudioSubmixEffectsChain> LDRSubmixEffectChain;
