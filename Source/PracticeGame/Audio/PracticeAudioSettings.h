...
#include "PracticeAudioSettings.generated.h"
...
USTRUCT()
struct PRACTICEGAME_API FPracticeSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "PracticeAudioSettings"))
class PRACTICEGAME_API UPracticeAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FPracticeSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FPracticeSubmixEffectChainMap> LDRAudioSubmixEffectChain;
