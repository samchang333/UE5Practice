...
#include "AbilitySystem/PracticeGameplayEffectContext.h"
#include "AbilitySystem/PracticeAbilitySourceInterface.h"
#include "Engine/World.h"
#include "Teams/PracticeTeamSubsystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UPracticeCombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UPracticeDamageExecution::UPracticeDamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}

void UPracticeDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FPracticeGameplayEffectContext* TypedContext = FPracticeGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	if (HitActor)
	{
		UPracticeTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UPracticeTeamSubsystem>();
	...
	if (const IPracticeAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UPracticeHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
