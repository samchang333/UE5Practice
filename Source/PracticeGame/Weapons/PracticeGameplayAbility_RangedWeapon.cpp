...
#include "PracticeGameplayAbility_RangedWeapon.h"
#include "Weapons/PracticeRangedWeaponInstance.h"
#include "Physics/PracticeCollisionChannels.h"
#include "PracticeLogChannels.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Weapons/PracticeWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/PracticeGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PracticeGameplayAbility_RangedWeapon)

namespace PracticeConsoleVariables
{
...
UPracticeGameplayAbility_RangedWeapon::UPracticeGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UPracticeRangedWeaponInstance* UPracticeGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UPracticeRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UPracticeGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogPracticeAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UPracticeRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UPracticeGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UPracticeGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UPracticeGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return Practice_TraceChannel_Weapon;
}

FHitResult UPracticeGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UPracticeGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UPracticeGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EPracticeAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UPracticeWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UPracticeWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EPracticeAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EPracticeAbilityTargetingSource::CameraTowardsFocus) || (Source == EPracticeAbilityTargetingSource::PawnTowardsFocus) || (Source == EPracticeAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EPracticeAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EPracticeAbilityTargetingSource::WeaponForward) || (Source == EPracticeAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EPracticeAbilityTargetingSource::PawnTowardsFocus) || (Source == EPracticeAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UPracticeGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (PracticeConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, PracticeConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UPracticeGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UPracticeRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EPracticeAbilityTargetingSource::CameraTowardsFocus);
		...
		if (PracticeConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, PracticeConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UPracticeGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UPracticeRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (PracticeConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, PracticeConsoleVariables::DrawBulletHitRadius, FColor::Red, false, PracticeConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UPracticeGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UPracticeGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UPracticeGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UPracticeWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UPracticeWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogPracticeAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UPracticeGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UPracticeWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UPracticeWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FPracticeGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FPracticeGameplayAbilityTargetData_SingleTargetHit();
