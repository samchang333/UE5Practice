...
#include "PracticeGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class UPracticeRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class EPracticeAbilityTargetingSource : uint8
{
...
/**
 * UPracticeGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UPracticeGameplayAbility_RangedWeapon : public UPracticeGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UPracticeGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="Practice|Ability")
	UPracticeRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UPracticeRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EPracticeAbilityTargetingSource Source) const;
