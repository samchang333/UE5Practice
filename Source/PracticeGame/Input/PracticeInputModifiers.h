...
#include "PracticeInputModifiers.generated.h"

struct FInputActionValue;

class FProperty;
class UEnhancedPlayerInput;
class UPracticeAimSensitivityData;
class UObject;
...
class UPracticeSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the Practice Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Practice Settings Driven Dead Zone"))
class UPracticeInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EPracticeTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in Practice Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Practice Gamepad Sensitivity"))
class UPracticeInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	EPracticeTargetingType TargetingType = EPracticeTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const UPracticeAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the Practice Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Practice Aim Inversion Setting"))
class UPracticeInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
