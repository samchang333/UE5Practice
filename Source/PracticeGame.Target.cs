...
public class PracticeGameTarget : TargetRules
{
	public PracticeGameTarget(TargetInfo Target) : base(Target)
	{
		...
		ExtraModuleNames.AddRange(new string[] { "PracticeGame" });
	...
	internal static void ApplySharedPracticeTargetSettings(TargetRules Target)
    {
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			PracticeGameTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
        {
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Log.TraceWarning("PracticeGameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}		
	}
